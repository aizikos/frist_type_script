import { ADD_Product, BASKET_PRODUCT, DELETE_BASKET, DELETE_Product, EDIT_Product, INCREMENT_QUANTITY, ProductActionType, UserState } from '../../types/types'
...
export const productReducer = (state = initialState, action: ProductActionType): UserState => {
    switch (action.type) {
        ...
        case BASKET_PRODUCT:
            ...
            let basket_res = state.basket.map((el) => el.id === action.payload.id ? { ...el, quently: el.quantly + 1 } : el)
        ...
        case INCREMENT_QUANTITY:
           // return { ...state, basket: state.basket.map((el) => el.id === action.payload.id ? { ...el, quently: el.quantly + 1 } : el) }
        case DELETE_BASKET:
            return {
